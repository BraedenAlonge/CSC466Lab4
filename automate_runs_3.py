...
def main():
    ...
    kmeans_params = {
        "data/4clusters.csv": 4,  # Known optimal for synthetic 4 clusters
        "data/mammal_milk.csv": 4,  # Changed from 3 to 4
        "data/planets.csv": 4,  # Changed from 3 to 4
        "data/iris.csv": 3,  # Standard for Iris dataset
        "data/AccidentsSet03.csv": 3  # Changed from 4 to 3 to avoid errors
    }

    # For hierarchical clustering: A dictionary mapping dataset filename to a threshold.
    hclustering_params = {
        "data/4clusters.csv": 7.0,  # Changed from 10.0 to 7.0 to force a 4-cluster split
        "data/mammal_milk.csv": 6.0,  # Changed from 5.0 to 6.0
        "data/planets.csv": 6.0,  # Changed from 5.0 to 6.0
        "data/iris.csv": 0.8,  # Changed from 1.0 to 0.8 (iris dendrogram is low)
        "data/AccidentsSet03.csv": 4.0  # Changed from 5.0 to 4.0
    }

    # For DBSCAN: A dictionary mapping dataset filename to a dictionary of epsilon and min_points.
    dbscan_params = {
        "data/4clusters.csv": {"epsilon": 0.7, "min_points": 4},  # Changed from 0.8, 3
        "data/mammal_milk.csv": {"epsilon": 1.2, "min_points": 4},  # Changed from 1.5, 3
        "data/planets.csv": {"epsilon": 8.0, "min_points": 3},  # Changed from 10.0, 2
        "data/iris.csv": {"epsilon": 0.55, "min_points": 4},  # Changed from 0.6, 5
        "data/AccidentsSet03.csv": {"epsilon": 0.7, "min_points": 4}  # Changed from 0.8, 3
    }
